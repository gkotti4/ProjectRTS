...
public class ControlGroupManager : MonoBehaviour
{
    ...
    private ControlGroup[] controlGroups;
    ...
    void Awake()
    {
        ...
        controlGroups = new ControlGroup[groupCount];
    ...
    public ControlGroup GetControlGroup(int groupIndex)
    {
    ...
    public void AssignControlGroup(int groupIndex)
    {
        ...
        ControlGroup group = controlGroups[groupIndex];
        group.members.Clear();

        foreach (ISelectable selectable in SelectionManager.Instance.GetSelectedObjects())
        {
            if (selectable == null) continue;
            if (selectable.GetGameObject() == null) continue;
            if (group.members.Contains(selectable)) continue;

            group.members.Add(selectable);
        }

        Debug.Log($"Assigned control group {groupIndex + 1}: {group.members.Count} objects");
    }
    ...
    public void SelectControlGroup(int groupIndex)
    {
        ...
        {
            ISelectable selectable = group.members[i];
            ...
            SelectionManager.Instance.SelectExternal(selectable);
        }
    }
    ...
    public void RemoveFromGroup(ISelectable selectable)
    {
        ...
        foreach (ControlGroup group in controlGroups)
        {
            ...
            group.members.Remove(selectable);
        }
    }

    public void ClearControlGroup(int groupIndex)
    {
        if (!IsValidGroupIndex(groupIndex))
            return;

        controlGroups[groupIndex].members.Clear();
    }

    public void ClearAllControlGroups()
    {
        foreach (ControlGroup group in controlGroups)
        {
            if (group == null) continue;
    ...
    bool IsValidGroupIndex(int groupIndex)
    {
        return groupIndex >= 0 &&
               controlGroups != null &&
               groupIndex < controlGroups.Length;
    }
