...
public class ActionButtonUI : MonoBehaviour
{
    ...
    public void InitializeCommand(CommandData command)
    {
        Clear();
        ...
        SetVisuals(command.icon, command.commandName, command.hotkey);
        SetButtonColor(affordableColor);
    }

    public void InitializeProduction(ProductionOptionData option)
    {
        Clear();
        ...
        SetVisuals(option.icon, option.productionName, option.hotkey);
    ...
    public void InitializeBuild(BuildOptionData option)
    {
        Clear();
        ...
        SetVisuals(option.icon, option.buildingName, option.hotkey);
        SetButtonColor(affordableColor);
    }
    ...
    void OnClick()
    {
        switch (mode)
        {
            case ButtonMode.Command:
                PlayerInputHandler.Instance.ExecuteSelectedCommand(commandData.commandType);
                return;
    ...
    void UpdateAffordability(FactionInstance factionInstance)
    {
        ...
        if (factionInstance != GameManager.Instance.PlayerFaction)
            return;
    ...
    void SetVisuals(Sprite icon, string text, HotkeySlot hotkey)
    {
        ...
        if (hotkeyLabel != null)
            hotkeyLabel.text = hotkey == HotkeySlot.None ? string.Empty : hotkey.ToString();
    }
