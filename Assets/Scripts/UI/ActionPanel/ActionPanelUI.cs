...
public class ActionPanelUI : MonoBehaviour
{
    ...
    public void ShowCommandPanel(ICommandable commandable)
    {
        currentWorker = commandable as WorkerController;
        ...
        List<CommandData> commands = commandable.GetCommands();
        ...
        foreach (CommandData command in commands)
        {
            ...
            if (command.hotkey == HotkeySlot.None)
                continue;
            ...
            {
                button.InitializeCommand(command);
            });
        }
    }

    public void ShowBuildingPanel(BuildingController building)
    {
        currentWorker = null;
        inBuildSubmenu = false;
        ...
        List<ProductionOptionData> options = building.GetProductionOptions();

        for (int i = 0; i < options.Count && i < maxButtons; i++)
        {
            ProductionOptionData option = options[i];
            ...
            FillSlot(i, button =>
            {
                button.InitializeProduction(option);
            });
        }
    }

    public void ShowBuildPanel(WorkerController worker)
    {
        ...
        foreach (BuildOptionData option in worker.Data.buildOptions)
        {
            if (option == null)
                continue;

            if (option.hotkey == HotkeySlot.None)
                continue;
    ...
    public void ExitBuildPanel()
    {
        ...
        if (currentWorker != null)
            ShowCommandPanel(currentWorker);
        else
            HidePanel();
    }
    ...
    void BuildButtonPool()
    {
        ...
        for (int i = 0; i < maxButtons; i++)
        {
            ...
            if (button == null)
            {
                Debug.LogError("Action button prefab missing ActionButtonUI.");
                continue;
            }
